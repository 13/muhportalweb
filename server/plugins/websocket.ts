...
export default defineNitroPlugin((nitroApp) => {
  ...
  // @ts-ignore - h3 internal API
  const server: Server | undefined = nitroApp.h3App.websocket?.server || nitroApp.h3App.nodeServer

  if (!server) {
    console.error('[WebSocket] Could not find HTTP server')
    return
  }
